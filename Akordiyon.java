/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.oop_proje_odevi;

/**
 *
 * @author daggu
 */
public class Akordiyon extends Tuslu_Calgilar {
    public Akordiyon(){}
    @Override
    public void Ozellikler(){
        System.out.println("Özellikler:\nAkordeon, akordiyon ya da akordion, bir körüğü harekete geçirmekle yaratılan hava akımının etkilediği serbest metal dillerinin titreşmesiyle ses çıkaran havalı çalgıdır.\nBir ya da iki kılavuz ile bir körükten oluşan akordiyonda, serbest metal dillerin titreşmesi, klavyenin tuşlarına basmakla sağlanır.");
    }
    public void Calma(){
        System.out.println("Nasıl Çalınır:\nAkordeon müzik aleti dilcik olarak adlandırılan hava baloncularından oluşmaktadır. Çalışma prensibi bu hava sayesinde oluşur.\nBir tarafından alınan hava akımı hareket ve temponun oluşturulması doğrultusunda ses titreşimleri meydana gelir.\nOrtaya çıkan bu ses titreşimleri diğer uçtan çıkması ise ses tonları ortaya çıkar.");
    }
}
